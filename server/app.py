...
class MemberOnlyArticle(Resource):
    ...
    def get(self, id):
        articles = [article.to_dict() for article in Article.query.filter(Article.id == id).all() if article.is_member_only == True]
        return make_response(jsonify(articles), 200)

@app.before_request
def check_if_logged_in():
    if not session.get('user_id') and request.endpoint not in ('clear', 'login','logout', 'show_article','article_list','check_session'):
